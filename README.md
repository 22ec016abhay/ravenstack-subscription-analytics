...
Excel — initial data audit (row counts, duplicates, nulls, sanity checks) before trusting the data enough to model it
SQL (PostgreSQL) — schema creation, data loading, and 7 sections of exploratory analysis: churn segmentation, revenue analysis, cohort/tenure analysis, feature usage & support correlation, and window-function-based queries (ROW_NUMBER, RANK, LAG, LEAD)
Power BI — star-schema data model, DAX measure library, and a 10-page interactive dashboard including a rule-based Customer Risk scoring system, a what-if churn scenario simulator, and drillthrough customer profiles
